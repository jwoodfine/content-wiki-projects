# Content contract — content-wiki-projects
...
> wiki server that renders this repo at `projects.woodfinegroup.com`.
>
...
| `aliases` | list | Old slugs that redirect to this article. Once published, never remove an alias. |
| `source_repo` | string | Declarative-mount only (Phase 6 planned). |
| `source_path` | string | Declarative-mount only (Phase 6 planned). |
| `edit_url` | string | Declarative-mount only (Phase 6 planned). |
| `references` | list | Only cited entries render into the bibliography. |
...
- **BCSC disclosure rule** (workspace §6) — Sovereign Data
  ...
  served at `projects.woodfinegroup.com`.

## 9. Gaps and migration state

Migration to the contract-conforming layout is in progress.
Open items are tracked in `NEXT.md` (local, gitignored).
...
Source: engine commit 2e0993e9 (feat: add GET /images/{*path} route).
