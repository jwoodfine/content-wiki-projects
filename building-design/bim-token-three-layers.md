...
paired_with: building-design/bim-token-three-layers.es.md
---
...
When a new BIM Token is authored, the intended authoring interface follows a four-zone model that maps directly to the three token layers plus a publishing workflow zone.
...
At v0.0.1, tokens are authored directly as DTCG JSON files and committed via git. The CMS authoring interface is a planned later-milestone deliverable.
