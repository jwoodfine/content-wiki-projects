...
paired_with: building-design/bim-floor-plate-methodology.es.md
---

The floor plate methodology is a geometric space-planning system in which every square foot of a building is assigned to a named Key Plan that traces directly to real furniture dimensions. A **[[bim-key-plans-index|Key Plan]]** is the smallest unit: a labelled rectangle around one tenant's actual furniture inventory, sized to enclose desks, storage, circulation, and accessibility clearances. A **[[bim-tile-system|Tile]]** is a composable group of Key Plans — each Tile is exactly one HVAC climate zone. A **Floor Plate** is the assembly of Tiles plus a Building Core plus Special Tiles; nothing on the floor is left unrepresented. Tile geometry follows the [[bim-building-width-method|Building Width Calculator]] and the [[bim-zone-depths-per-use-type|zone depths per use type]]; named assembly examples are catalogued in [[bim-floor-plate-tile-combinations|floor-plate tile combinations]].
...
Special Tiles SP-A, SP-B, SP-C fill the residual area surrounding the Core. Their width is snapped to the nearest [[bim-professional-office-key-plans|Professional Office Key Plan]] width. This preserves demising-wall continuity. SP-C, in front of the Core, must avoid direct alignment of any door with the elevator opening (V12 Methodology p. 9).
