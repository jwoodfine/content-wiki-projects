...
**Layer 1: Semantic identity via bSDD.** Every [[topic-bim-objects-what-they-are|BIM Object]] carries a bSDD concept URI identifying its element type in a jurisdiction-neutral, tool-neutral reference. This URI is the stable identity that allows Regulation and Climate Zone overlays to reference the same element type regardless of IFC version drift.
...
The composition of these [[topic-bim-objects-three-layers|three layers]] is what makes the geometry "encode" the code. The regulatory constraint is not stored in a separate validation database checked after authoring. It is stored in the BIM Object specification and instantiated with the element.
