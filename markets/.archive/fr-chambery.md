---
schema: foundry-doc-v1
slug: fr-chambery
title: "Chambéry Regional Market"
language: en
language_protocol: PROSE-TOPIC
category: markets
index_group: market-profiles
type: reference
content_type: topic
quality: stub
status: archived
archived: 2026-09-05
archived_reason: "Retired market profile, archived 2026-09-05: retired 2026-08-17 because France is deliberately excluded from the current Regional Markets methodology (locked 2026-06-30), so this profile describes no market in the current Top 400 or reference-only sets. It had been left live under a Superseded and Retired shelf on the category index; that shelf is retired with this pass and the page moves to the archive."
superseded_by: none
audience: customer-woodfine
bcsc_class: current-fact
short_description: "European Regional Market — suburb of Grenoble; 1 Regional-tier and 1 Local-tier co-location cluster."
paired_with: markets/fr-chambery.es.md
last_edited: 2026-08-25
editor: editorial
---

Chambéry, the prefecture of the Savoie department and a major Alpine railway hub, is a
suburb of Grenoble with one Regional-tier and one Local-tier co-location cluster. The city's
character — administrative, educational, and transport-oriented rather than a conventional
retail destination — is atypical among the covered markets that carry a Regional-tier
cluster.

## Overview

According to Wikipedia (accessed 2026-07-10), the Chambéry commune had a population of
59,964 in 2023, with the wider metropolitan area reaching 263,919 in 2022, ranking as
France's 94th-largest city. The commune covers 20.99 square kilometres, with an urban area
of 327.5 square kilometres. Wikipedia's article does not state the distance to Grenoble
directly, but notes Chambéry sits 523 kilometres from Paris and 100 kilometres from Lyon.
The city is a major railway hub at the midpoint of the Franco-Italian Turin–Lyon high-speed
line and hosts university campuses as the seat of the Savoie prefecture. Chambéry holds an
AOC designation for vermouth production, with the Dolin and Routin brands based there — a
regional specialty rather than a national retail draw. Wikipedia's coverage emphasises the
city's historical landmarks and tourism character over modern commercial development.

## Regional Market Standing

Chambéry is a suburb of Grenoble, with one Regional-tier co-location cluster and one
Local-tier co-location cluster.

Cluster-level detail — the specific retail and civic anchors composing each of Chambéry's
co-location clusters — is pending the underlying GIS dataset and is not included in this
stub. See [[atlas-top-400-europe|Top 400 Regional Markets — Europe]] for the full Top 400
set and [[co-location-methodology|Co-location Methodology]] for how markets qualify.

## See Also

- [[atlas-top-400-europe|Top 400 Regional Markets — Europe]]
- [[about-regional-markets-system|Regional Markets Intelligence System]]
- [[co-location-methodology|Co-location Methodology]]

## References

- [Chambéry](https://en.wikipedia.org/wiki/Chamb%C3%A9ry) — Wikipedia, accessed 2026-07-10

*Wikipedia content reproduced under [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).*
